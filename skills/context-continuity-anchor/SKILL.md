...
description: 内部 continuity engine。默认由 agent-continuity-harness 在命中 continuity 条件后路由进入；通常经由 adg 的 guard-mode 升级而来，负责分层判断、状态治理与承接恢复。Use directly only when the user explicitly wants cca or the task itself is maintaining continuity rules, state roots, or recovery.
---
