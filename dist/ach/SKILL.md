...
2. 若没有，默认进入 `adg`，并按 [guard 规则](./references/adg/guard.md) 的 `guard-mode` 工作。
3. 若有，则进入 `cca`，并按 [continuity 入口规则](./references/cca/entry.md) 的启动协议读取 `.cca-bindings.json` 与当前任务正式状态根。
